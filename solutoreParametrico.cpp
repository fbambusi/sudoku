...
void importa(int numeroSchema,int contenitore[dim][dim],int brutta[dim][dim][dim]){//i parenti certi
    ...
    if(ingresso!=NULL){//il file è stato aperto
     ...
     while(riga<dim){
     	colonna=0;
     	while(colonna<dim){//in larghezza pesa 10 perché che il \n
...
int haParentiCerti(int schemino[dim][dim],int riga,int colonna){//ritorna 1 se ha parenti certi, 0 se non ne ha
	int cont,rigaAux,colAux;
	for(cont=0;cont<dimensione;cont++){
...
void risolvi3(lista controllo,int bella[dim][dim],int brutta[dim][dim][dim],int livHp){//trae le conclusioni su uno schema ipotetico
int numeroSchema;
int riga;
int colonna;
int cont;
...
else if ((brutta[numeroSchema][riga][colonna]==si)){
...
int messo=0;//diventa 1 non appena 
...
//printf("è una riga %d\n",indiceMin+1);
	...
	for(colonna=0;colonna<dim&&messo==0;colonna++){//su tutte le colonne della riga
		for(numeroSchema=0;numeroSchema<dim;numeroSchema++){
...
risolvi3(controllo,bella,brutta,livHp);//ho tratto tutte le conclusioni possibili
...
printf("Inserisci il numero dello schema di cui desideri la soluzione e premi INVIO.\n");
